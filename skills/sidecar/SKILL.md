...
- **`plugin-not-found`** — the plugin isn't installed. Tell the user to install the Sidecar plugin first.
- **`needs-setup`** — plugin installed but no state dir / no `.env.local`. Run `setup.sh`, then prompt for the OpenRouter key.
...
```bash
bash "$SIDECAR_SKILL/scripts/list-models.sh" claude opus
```
...
if [ "$COMPLETION_MODEL" != "$DESIRED" ]; then
  bash "$SIDECAR_SKILL/scripts/set-model.sh" "$DESIRED"
fi
```
...
```bash
bash "$SIDECAR_SKILL/scripts/ask.sh" "<the user's prompt verbatim>"
```

Or via stdin (better for prompts with quotes/special chars):

```bash
echo "<the user's prompt verbatim>" | bash "$SIDECAR_SKILL/scripts/ask.sh"
```
...
- For full control, drop down to `scripts/start.sh` + multiple `claude -p` calls in one bash invocation (proxy stays warm).
...
```bash
bash "$SIDECAR_SKILL/scripts/list-models.sh" claude sonnet
bash "$SIDECAR_SKILL/scripts/set-model.sh" anthropic/claude-sonnet-4.6
```
...
```bash
bash "$SIDECAR_SKILL/scripts/status.sh"
```
