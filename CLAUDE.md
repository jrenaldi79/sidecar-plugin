...
- **Timeouts**: Cowork's bash tool kills invocations at ~45s regardless of `ask.sh`'s `MAX_RUN_SECONDS` (default 180). Long calls (always compare.sh): launch with `run_in_background: true` and read via TaskOutput; or use a real terminal. On stock macOS `timeout(1)` may be absent — ask.sh and test.sh fall back timeout → gtimeout → unguarded; any NEW script invoking timeout must reuse that chain (bare `timeout` broke test.sh on Macs for a day).
...
- **`bundle.cjs` is gitignored; `bundle-min.cjs` IS tracked and is what git-clone/marketplace installs run** (`_locate.sh` resolves `SIDECAR_BUNDLE_OVERRIDE` > `bundle.cjs` > `bundle-min.cjs`). `build.sh` regenerates BOTH — any commit touching `anthropic-proxy-patched.mjs`/`wrapper.mjs` must run `build.sh` and commit the refreshed `bundle-min.cjs`, or remote installs ship a stale proxy. Never hand-edit either bundle. Verify the min bundle functionally: `SIDECAR_TEST_ENTRY=$PWD/skills/sidecar/proxy/bundle-min.cjs bash tests/run-integration.sh`.
- **OpenRouter providers stream `tool_calls[].function.arguments` in two incompatible styles** — cumulative full-string-so-far (Gemini) vs incremental fragments (OpenAI). PATCH P3's `startsWith` heuristic in the proxy normalizes both; never assume either style when touching the streaming path.
